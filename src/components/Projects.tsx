...
const Projects = () => {
  ...
  const projects = [
    {
      title: "GradeMate – Automated grading system for exams",
      description: "A system that automatically grades exams and provides instant feedback to students.",
      tags: ["Python", "FastAPI", "React", "Gemini", "MySQL"],
      feature: true,
      links: [
        { type: "Code", url: "#" },
        { type: "Demo", url: "grademate.vercel.app" },
      ],
    },
    {
      title: "GlobalNest – A platform for migrants to adjust to new environments",
      description: "A platform that connects newcomers with verified local mentors, helping them adapt faster to culture, language, and daily tasks.",
      tags: ["React", "Spring Boot", "MySQL"],
      feature: true,
      links: [
        { type: "Code", url: "#" },
        { type: "Demo", url: "globalnestt.vercel.app" },
      ],
    },
    {
          ...
          {projects.map((project, index) => {
            ...
            return (
              <div
                key={index}
                ref={ref}
                className={`bg-card border border-border rounded-lg p-8 hover:border-primary transition-all duration-500 group cursor-pointer hover:scale-105 hover:-translate-y-3 card-glow-hover relative overflow-hidden ${project.featured ? "md:col-span-1" : ""
                  ...
                  <div className="flex gap-4 h-10 w-full">
                    {project.links.map((link, linkIndex) => (
                      <Button
                        ...
                        variant="outline"
                        size="sm"
                        className="group/btn hover:scale-110 transition-all duration-300 flex-1"
                        onClick={() => window.open(link.url.startsWith("http") || link.url === "#" ? link.url : `https://${link.url}`, "_blank")}
                      >
                        {link.type === "Code" ? (
